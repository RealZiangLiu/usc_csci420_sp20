  ...
  */
  ...
  for (int i=0; i<numSplines; ++i) {
    ...
    for (int j=0; j<uNumPoints-1; ++j) {
      for (int k=0; k<6; ++k) {
          // bottom right: right
          squareColors[j*4+k] = glm::vec4(splineBinormals[i][j].x, splineBinormals[i][j].y, splineBinormals[i][j].z, 1.0);
          // top right: top
          squareColors[j*4+1*6+k] = glm::vec4(splineNormals[i][j].x, splineNormals[i][j].y, splineNormals[i][j].z, 1.0);
          // top left: left
          squareColors[j*4+2*6+k] = glm::vec4(-splineBinormals[i][j].x, -splineBinormals[i][j].y, -splineBinormals[i][j].z, 1.0);
          // bottom left: bottom
          squareColors[j*4+3*6+k] = glm::vec4(-splineNormals[i][j].x, -splineNormals[i][j].y, -splineNormals[i][j].z, 1.0);
      }
    }
