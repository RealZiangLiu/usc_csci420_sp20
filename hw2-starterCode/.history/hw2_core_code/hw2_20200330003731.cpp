...
void loadCrossbarTexture (glm::vec3* crossbarTrianglePositions) {
  ...
  for (int i=0; i<crossbar_cnt; i+=2) {
    for (int j=0; j<4; ++j) {
      crossbarTexCoords[i * 24 + j * 6] = glm::vec2(1, 0);
      crossbarTexCoords[i * 24 + j * 6 + 1] = glm::vec2(1, 1);
      crossbarTexCoords[i * 24 + j * 6 + 2] = glm::vec2(0, 0);
      crossbarTexCoords[i * 24 + j * 6 + 3] = glm::vec2(1, 1);
      crossbarTexCoords[i * 24 + j * 6 + 4] = glm::vec2(0, 1);
      crossbarTexCoords[i * 24 + j * 6 + 5] = glm::vec2(0, 0);
    }
  }
  // ============================= Bind texture VAO and VBO ========================
  // Set positions VBO
