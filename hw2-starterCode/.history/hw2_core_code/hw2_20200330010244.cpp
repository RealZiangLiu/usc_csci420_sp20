...
const int crossbar_dist = 450;
const int crossbar_width = 120;
...
void loadCrossbarTexture (glm::vec3* crossbarTrianglePositions) {
  ...
  for (int i=0; i<crossbar_cnt; i+=2) {
    for (int j=0; j<4; ++j) {
      crossbarTexCoords[i * 12 + j * 6] = glm::vec2(1, 0);
      crossbarTexCoords[i * 12 + j * 6 + 1] = glm::vec2(1, 1);
      crossbarTexCoords[i * 12 + j * 6 + 2] = glm::vec2(0, 0);
      crossbarTexCoords[i * 12 + j * 6 + 3] = glm::vec2(1, 1);
      crossbarTexCoords[i * 12 + j * 6 + 4] = glm::vec2(0, 1);
      crossbarTexCoords[i * 12 + j * 6 + 5] = glm::vec2(0, 0);
    }
  }
  // ============================= Bind texture VAO and VBO ========================
  // Set positions VBO
...
void displayFunc()
{
  ...
  if (!camera_on_rail) {
    ...
    matrix.LookAt(splinePointCoords[0][roller_frame_count].x + 0.11 * splineNormals[0][roller_frame_count].x, 
                  splinePointCoords[0][roller_frame_count].y + 0.11 * splineNormals[0][roller_frame_count].y,
                  splinePointCoords[0][roller_frame_count].z + 0.11 * splineNormals[0][roller_frame_count].z, // eye point
                  splinePointCoords[0][focusIdx].x + 0.11 * splineNormals[0][focusIdx].x, 
                  splinePointCoords[0][focusIdx].y + 0.11 * splineNormals[0][focusIdx].y,
                  splinePointCoords[0][focusIdx].z + 0.11 * splineNormals[0][focusIdx].z,          // focus point          
...
void add_cross_bar_points (vector<Point>& crossbarPositions, int splineIdx, int pointCnt) {
  ...
  for (int i=crossbar_dist / 2 + crossbar_width; i<pointCnt; i+=crossbar_dist) {
    ...
    crossbarPositions.push_back(Point(v_5.x, v_5.y, v_5.z));
