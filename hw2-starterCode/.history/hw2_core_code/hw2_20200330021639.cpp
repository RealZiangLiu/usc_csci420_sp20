...
void loadSkyTexture () {
  ...
  int pos_cnt = 0;
  // Top
  groundPositions[pos_cnt++] = glm::vec3(500, 990, 500);
  groundPositions[pos_cnt++] = glm::vec3(500, 990, -500);
  groundPositions[pos_cnt++] = glm::vec3(-500, 990, -500);
  groundPositions[pos_cnt++] = glm::vec3(500, 990, 500);
  groundPositions[pos_cnt++] = glm::vec3(-500, 990, 500);
  ...
  // Left
  groundPositions[pos_cnt++] = glm::vec3(-500, -10, 500);
  groundPositions[pos_cnt++] = glm::vec3(-500, -10, -500);
  groundPositions[pos_cnt++] = glm::vec3(-500, 990, -500);
  groundPositions[pos_cnt++] = glm::vec3(-500, -10, 500);
  groundPositions[pos_cnt++] = glm::vec3(-500, 990, 500);
  ...
  for (int i=0; i<5; ++i) {
    groundTexCoords[i * 6 + 0] = glm::vec2(1, 0);
    groundTexCoords[i * 6 + 1] = glm::vec2(1, 1);
    groundTexCoords[i * 6 + 2] = glm::vec2(0, 1);
    groundTexCoords[i * 6 + 3] = glm::vec2(1, 0);
    groundTexCoords[i * 6 + 4] = glm::vec2(0, 0);
    groundTexCoords[i * 6 + 5] = glm::vec2(0, 1);
  }
  // ============================= Bind texture VAO and VBO ========================
  // Set positions VBO
  glGenBuffers(1, &groundVBO);
  ...
  // Upload position data
  glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3) * 30, groundPositions);
  // Upload texCoord data
  glBufferSubData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * 30, sizeof(glm::vec2) * 30, groundTexCoords);
  // Set VAO
  glGenVertexArrays(1, &groundVAO);
  glBindVertexArray(groundVAO);
  // Bind vbo
