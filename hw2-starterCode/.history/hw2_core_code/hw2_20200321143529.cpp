...
void renderSplines () {
  for (size_t i=0; i<numSplines; ++i) {
    ...
    glDrawElements(GL_TRIANGLES, splineVertexCnt[i], GL_UNSIGNED_INT, (void*)0);
    glBindVertexArray(0);
  }
}
...
void compute_square_rail_idx (unsigned int* squareIdx, int pointCnt) {
  ...
  for (int i=0; i<pointCnt-1; ++i) {
    ...
    // right
    squareIdx[currCnt++] = i + 0;
    squareIdx[currCnt++] = i + 1;
    squareIdx[currCnt++] = i + 4;
    squareIdx[currCnt++] = i + 1;
    squareIdx[currCnt++] = i + 4;
    squareIdx[currCnt++] = i + 5;
    // top
    squareIdx[currCnt++] = i + 1;
    squareIdx[currCnt++] = i + 2;
    squareIdx[currCnt++] = i + 5;
    squareIdx[currCnt++] = i + 2;
    squareIdx[currCnt++] = i + 5;
    squareIdx[currCnt++] = i + 6;
    // left
    squareIdx[currCnt++] = i + 2;
    squareIdx[currCnt++] = i + 3;
    squareIdx[currCnt++] = i + 6;
    squareIdx[currCnt++] = i + 3;
    squareIdx[currCnt++] = i + 6;
    squareIdx[currCnt++] = i + 7;
    // bottom
    squareIdx[currCnt++] = i + 3;
    squareIdx[currCnt++] = i + 0;
    squareIdx[currCnt++] = i + 7;
...
void initScene(int argc, char *argv[])
{
  ...
  */
  ...
  for (int i=0; i<numSplines; ++i) {
    ...
    splineVertexCnt.push_back(uNumPoints * 4);
