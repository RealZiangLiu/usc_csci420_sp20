...
void displayFunc()
{
  ...
  if (!camera_on_rail) {
    ...
    matrix.SetMatrixMode(OpenGLMatrix::ModelView);
    matrix.LoadIdentity();
    matrix.LookAt(splinePointCoords[0][roller_frame_count].x, 
                  splinePointCoords[0][roller_frame_count].y,
                  splinePointCoords[0][roller_frame_count].z, // eye point
  ...
  // DEBUG print
  cout << "Coord: " << splineTangents[0][roller_frame_count].x << endl;
...
void initScene(int argc, char *argv[])
{
  ...
  */
  ...
  for (int i=0; i<numSplines; ++i) {
    ...
    Point T_0 = splineTangents[i][0];
    Point N_0 = T_0.cross(initial_V);
    Point B_0 = T_0.cross(N_0);
    splineNormals[i].push_back(N_0);
    splineNormals[i].push_back(B_0);
    for (int i=1; i<uNumPoints; ++i) {
