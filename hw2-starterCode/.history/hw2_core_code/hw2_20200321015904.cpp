...
void add_square_rail_points (glm::vec3* pointPositions, int splineIdx, int pointCnt, Point& p_0) {
  for (int i=1; i<splineVertexCnt[splineIdx]; ++i) {
    ...
    v_4 = p_1 + (b_1 - n_1) * param_rail_scale;
    v_5 = p_1 + (n_1 + b_1) * param_rail_scale;
    v_6 = p_1 + (n_1 - b_1) * param_rail_scale;
    v_7 = p_1 + (Point(0,0,0) - n_1 - b_1) * param_rail_scale;
...
void compute_store_points_tangents (glm::vec3* pointPositions, int splineIdx, int pointCnt, int u_cnt, Point& p_1, Point& p_2, Point& p_3, Point& p_4) {
  ...
  // TODO: compute four points for square cross section
...
void compute_catmull_rom_point (glm::vec3* pointPositions, glm::vec3* squarePositions, Point* points, int currNumCtrlPts, int splineIdx, Point& prev_1, Point& prev_2, Point& next_1, bool connect_prev = false, bool connect_next = false) {
  ...
  // TODO: for each point compute four points for square
}
