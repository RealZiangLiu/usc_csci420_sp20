...
void compute_square_rail_idx (glm::vec3* squareIdx, glm::vec3* squarePositions, int splineIdx, ) {
  for (int i=1; i<)
}
...
void compute_catmull_rom_point (glm::vec3* pointPositions, glm::vec3* squarePositions, Point* points, int currNumCtrlPts, int splineIdx, Point& prev_1, Point& prev_2, Point& next_1, bool connect_prev = false, bool connect_next = false, int& pointCnt) {
  pointCnt = 0;
...
void initScene(int argc, char *argv[])
{
  ...
  */
  ...
  for (int i=0; i<numSplines; ++i) {
    ...
    compute_catmull_rom_point(pointPositions, squarePositions, splines[i].points, currNumCtrlPts, i, prev_1_point, prev_2_point, next_1_point, connect_prev, connect_next, realPointCnt);
  

    // Set colors
    for (int i=0; i<realPointCnt; ++i) {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * realPointCnt + sizeof(glm::vec4) * realPointCnt, nullptr, GL_STATIC_DRAW);
    // Upload position data
    // glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(pointPositions), pointPositions);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3) * realPointCnt, pointPositions);
    // Upload color data
    // glBufferSubData(GL_ARRAY_BUFFER, sizeof(pointPositions), sizeof(pointColors), pointColors);
    glBufferSubData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * realPointCnt, sizeof(glm::vec4) * realPointCnt, pointColors);
    ...
    offset = (const void*) (sizeof(glm::vec3) * realPointCnt);
    ...
    splineVertexCnt.push_back(realPointCnt);
