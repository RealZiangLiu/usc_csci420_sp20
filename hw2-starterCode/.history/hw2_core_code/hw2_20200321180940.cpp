  ...
  */
  ...
  for (int i=0; i<numSplines; ++i) {
    ...
    for (int j=0; j<uNumPoints-1; ++j) {
      for (int k=0; k<6; ++k) {
          // bottom right: right
          squareColors[j*24+k] = glm::vec4(splineBinormals[i][j].x, splineBinormals[i][j].y, splineBinormals[i][j].z);
          // top right: top
          squareColors[j*24+1*6+k] = glm::vec4(splineNormals[i][j].x, splineNormals[i][j].y, splineNormals[i][j].z);
          // top left: left
          squareColors[j*24+2*6+k] = glm::vec4(-splineBinormals[i][j].x, -splineBinormals[i][j].y, -splineBinormals[i][j].z);
          // bottom left: bottom
          squareColors[j*24+3*6+k] = glm::vec4(-splineNormals[i][j].x, -splineNormals[i][j].y, -splineNormals[i][j].z);
      }
    }
