...
const double param_rail_scale = 0.003;
...
void displayFunc()
{
  ...
  if (!camera_on_rail) {
    ...
    matrix.LookAt(splinePointCoords[0][roller_frame_count].x + 0.04 * splineNormals[0][roller_frame_count].x, 
                  ...
                  splinePointCoords[0][focusIdx].x + 0.03 * splineNormals[0][focusIdx].x, 
                  splinePointCoords[0][focusIdx].y + 0.03 * splineNormals[0][focusIdx].y,
                  splinePointCoords[0][focusIdx].z + 0.03 * splineNormals[0][focusIdx].z,          // focus point          
...
void renderSplines () {
  for (size_t i=0; i<numSplines; ++i) {
    ...
    cout << "In render: " << splineSquareEBOCnt[i] << endl;
    glBindVertexArray(0);
  }
}
...
void initScene(int argc, char *argv[])
{
  ...
  */
  ...
  for (int i=0; i<numSplines; ++i) {
    ...
    GLuint currVBO, currVAO, currEBO;
    ...
    glm::vec4* squareColors = new glm::vec4[uNumPoints*4];
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * uNumPoints * 4 + sizeof(glm::vec4) * uNumPoints * 4, nullptr, GL_STATIC_DRAW);
    // Upload position data
    // glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3) * uNumPoints, pointPositions);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3) * uNumPoints * 4, squarePositions);
    // Upload color data
    // glBufferSubData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * uNumPoints, sizeof(glm::vec4) * uNumPoints, pointColors);
    glBufferSubData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * uNumPoints * 4, sizeof(glm::vec4) * uNumPoints * 4, squareColors);
    ...
    offset = (const void*) (sizeof(glm::vec3) * uNumPoints * 4);
