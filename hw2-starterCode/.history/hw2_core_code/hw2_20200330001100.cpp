...
const int crossbar_dist = 20;
const int crossbar_width = 10;
...
void loadCrossbarTexture (glm::vec3* crossbarPositions) {
  // TODO
  glGenTextures(1, &crossbarHandle);

  int code = initTexture("texture_oldWood.jpg", crossbarHandle);
  if (code != 0) {
    printf("Error loading the texture image.\n");
    exit(EXIT_FAILURE); 
  }

  int crossbar_vertex_cnt = crossbar_cnt * 12;
  
  glm::vec2* crossbarTexCoords = new glm::vec2[crossbar_cnt * 12];

  // Populate positions array
  for (int i=0; i<crossbar_cnt; i+=2) {
    for (int j=0; j<4; ++j) {
      crossbarTexCoords[i * 2 + j * 6] = glm::vec2(1, 0);
      crossbarTexCoords[i * 2 + j * 6 + 1] = glm::vec2(1, 1);
      crossbarTexCoords[i * 2 + j * 6 + 2] = glm::vec2(0, 0);
      crossbarTexCoords[i * 2 + j * 6 + 3] = glm::vec2(1, 1);
      crossbarTexCoords[i * 2 + j * 6 + 4] = glm::vec2(0, 1);
      crossbarTexCoords[i * 2 + j * 6 + 5] = glm::vec2(0, 0);
    }
  }
  // ============================= Bind texture VAO and VBO ========================
  // Set positions VBO
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * crossbar_cnt * 12 + sizeof(glm::vec2) * 6, nullptr, GL_STATIC_DRAW);
  // Upload position data
  glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3) * 6, groundPositions);
  // Upload texCoord data
  glBufferSubData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * 6, sizeof(glm::vec2) * 6, groundTexCoords);
...
void add_cross_bar_points (glm::vec3* crossbarPositions, int splineIdx, int pointCnt) {
  ...
  for (int i=crossbar_dist / 2 + crossbar_width; i<pointCnt; i+=crossbar_dist) {
    ...
    crossbarPositions[crossBarPointCnt++] = glm::vec3(v_0.x, v_0.y, v_0.z);
    crossbarPositions[crossBarPointCnt++] = glm::vec3(v_1.x, v_1.y, v_1.z);
    crossbarPositions[crossBarPointCnt++] = glm::vec3(v_2.x, v_2.y, v_2.z);
    crossbarPositions[crossBarPointCnt++] = glm::vec3(v_3.x, v_3.y, v_3.z);
    crossbarPositions[crossBarPointCnt++] = glm::vec3(v_4.x, v_4.y, v_4.z);
    crossbarPositions[crossBarPointCnt++] = glm::vec3(v_5.x, v_5.y, v_1.z);
    crossbarPositions[crossBarPointCnt++] = glm::vec3(v_6.x, v_6.y, v_6.z);
    crossbarPositions[crossBarPointCnt++] = glm::vec3(v_7.x, v_7.y, v_7.z);
...
void compute_cross_bar_idx (glm::vec3* crossbarTrianglePositions, glm::vec3* crossbarPositions) {
  int currCnt = 0;
  for (int i=0; i<crossbar_cnt; i+=2) {
    // right
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 0];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 4];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 1];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 4];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 5];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 1];
    // top
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 1];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 5];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 2];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 5];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 6];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 2];
    // left
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 2];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 6];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 3];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 6];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 7];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 3];
    // bottom
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 3];
    crossbarTrianglePositions[currCnt++] = crossbarPositions[i * 4 + 7];
...
void compute_catmull_rom_point (glm::vec3* pointPositions, glm::vec3* squarePositions, glm::vec3* crossbarPositions, Point* points, int currNumCtrlPts, int splineIdx, Point& prev_1, Point& prev_2, Point& next_1, bool connect_prev, bool connect_next) {
  int pointCnt = 0;
...
void initScene(int argc, char *argv[])
{
  ...
  */
  ...
  for (int i=0; i<numSplines; ++i) {
    ...
    glm::vec3* crossbarTrianglePositions = new glm::vec3[squareIdxCnt];
