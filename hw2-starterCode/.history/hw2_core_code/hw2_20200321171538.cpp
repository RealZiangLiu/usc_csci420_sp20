...
void loadGroundTexture () {
  ...
  groundPositions[0] = glm::vec3(100, -50, 100);
  groundPositions[1] = glm::vec3(100, -50, -100);
  groundPositions[2] = glm::vec3(-100, -50, -100);
  groundPositions[3] = glm::vec3(100, -50, 100);
  groundPositions[4] = glm::vec3(-100, -50, 100);
  groundPositions[5] = glm::vec3(-100, -50, -100);
...
void displayFunc()
{
  ...
  GLint h_modelViewMatrix = glGetUniformLocation(milestoneProgram, "modelViewMatrix"); 
  // get a handle to the projectionMatrix shader variable
  GLint h_projectionMatrix = glGetUniformLocation(milestoneProgram, "projectionMatrix"); 
  // Get handle to mode shader variable
  GLint h_mode = glGetUniformLocation(milestoneProgram, "mode");
  ...
  // get a handle to the modelViewMatrix shader variable
  GLint h_modelViewMatrix = glGetUniformLocation(textureProgram, "modelViewMatrix"); 
  // get a handle to the projectionMatrix shader variable
  GLint h_projectionMatrix = glGetUniformLocation(textureProgram, "projectionMatrix"); 
  // Get handle to mode shader variable
  GLint h_mode = glGetUniformLocation(textureProgram, "mode");
  ...
  texturePipelineProgram->setTextureUnit(GL_TEXTURE0); // it is safe to always use GL_TEXTURE0
