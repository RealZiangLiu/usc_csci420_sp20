...
void compute_store_points_tangents (glm::vec3* pointPositions, int pointCnt, int u_cnt, Point& p_1, Point& p_2, Point& p_3, Point& p_4) {
...
void compute_catmull_rom_point (glm::vec3* pointPositions, Point* points, int currNumCtrlPts, int splineIdx, Point& prev_1, Point& prev_2, Point& next_1, bool connect_prev = false, bool connect_next = false) {
  ...
  if (connect_prev) {
    ...
    for (int u_cnt=0; u_cnt < (int)(1.0 / param_u_step); ++u_cnt) {
      ...
      // // Position vector to put into VBO
      // pointPositions[pointCnt] = glm::vec3(res.x, res.y, res.z);
      compute_store_points_tangents(pointPositions, pointCnt, u_cnt, prev_2, prev_1, points[1], points[2]);
    ...
    for (int u_cnt=0; u_cnt < (int)(1.0 / param_u_step); ++u_cnt) {
      ...
      compute_store_points_tangents(pointPositions, pointCnt, u_cnt, prev_1, points[1], points[2], points[3]);
  ...
  for (int i=start; i<end; ++i) {
    for (int u_cnt=0; u_cnt < (int)(1.0 / param_u_step); ++u_cnt) {
      ...
      compute_store_points_tangents(pointPositions, pointCnt, u_cnt, points[i-1], points[i], points[i+1], points[i+2]);
  ...
  if (connect_next) {
    ...
    for (int u_cnt=0; u_cnt <= (int)(1.0 / param_u_step); ++u_cnt) {
      ...
      compute_store_points_tangents(pointPositions, pointCnt, u_cnt, points[currNumCtrlPts-4], points[currNumCtrlPts-3], points[currNumCtrlPts-2], next_1);
    ...
    compute_store_points_tangents(pointPositions, pointCnt, (int)(1.0 / param_u_step), points[currNumCtrlPts-4], points[currNumCtrlPts-3], points[currNumCtrlPts-2], points[currNumCtrlPts-1]);
