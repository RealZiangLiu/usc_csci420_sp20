...
void compute_square_rail_idx (unsigned int* squareIdx, glm::vec3* squarePositions, int splineIdx, int pointCnt) {
  ...
  for (int i=0; i<pointCnt-1; ++i) {
    // Four surfaces: right - top - left - bottom
    for (int j=0; j<4; ++j) {
      squareIdx[currCnt++] = i + j;
      squareIdx[currCnt++] = i + (j + 1) % 4;
      squareIdx[currCnt++] = i + j + 4;
      squareIdx[currCnt++] = i + j + 1;
      squareIdx[currCnt++] = i + j + 4;
      squareIdx[currCnt++] = i + j + 5;
    }
  }
}
