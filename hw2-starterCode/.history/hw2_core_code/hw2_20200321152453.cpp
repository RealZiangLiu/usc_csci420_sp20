...
void renderSplines () {
  for (size_t i=0; i<numSplines; ++i) {
    ...
    // glDrawArrays(GL_TRIANGLES, 0, splineVertexCnt[i]);
    glDrawElements(GL_TRIANGLES, splineSquareEBOCnt[i], GL_UNSIGNED_INT, (void*)0);
    glBindVertexArray(0);
  }
}

void add_square_rail_points (glm::vec3* squareTrianglePositions, glm::vec3* squarePositions, int splineIdx, int pointCnt) {
...
// TODO: get rid of EBO part
void compute_square_rail_idx (unsigned int* squareIdx, int pointCnt) {
  int currCnt = 0;
  for (int i=0; i<pointCnt-1; ++i) {
    // TODO: change these into a for loop
    // right
    squareIdx[currCnt++] = i * 4 + 0;
    squareIdx[currCnt++] = i * 4 + 4;
    squareIdx[currCnt++] = i * 4 + 1;
    squareIdx[currCnt++] = i * 4 + 4;
    squareIdx[currCnt++] = i * 4 + 5;
    squareIdx[currCnt++] = i * 4 + 1;
    // top
    squareIdx[currCnt++] = i * 4 + 1;
    squareIdx[currCnt++] = i * 4 + 5;
    squareIdx[currCnt++] = i * 4 + 2;
    squareIdx[currCnt++] = i * 4 + 5;
    squareIdx[currCnt++] = i * 4 + 6;
    squareIdx[currCnt++] = i * 4 + 2;
    // left * 4
    squareIdx[currCnt++] = i * 4 + 2;
    squareIdx[currCnt++] = i * 4 + 6;
    squareIdx[currCnt++] = i * 4 + 3;
    squareIdx[currCnt++] = i * 4 + 6;
    squareIdx[currCnt++] = i * 4 + 7;
    squareIdx[currCnt++] = i * 4 + 3;
    // bottom * 4
    squareIdx[currCnt++] = i * 4 + 3;
    squareIdx[currCnt++] = i * 4 + 7;
...
void compute_catmull_rom_point (glm::vec3* pointPositions, glm::vec3* squarePositions, glm::vec3* squareTrianglePositions, Point* points, int currNumCtrlPts, int splineIdx, Point& prev_1, Point& prev_2, Point& next_1, bool connect_prev, bool connect_next) {
  int pointCnt = 0;
  ...
  // TODO: check this
  add_square_rail_points(squareTrianglePositions, squarePositions, splineIdx, pointCnt);
}
...
void initScene(int argc, char *argv[])
{
  ...
  */
  ...
  for (int i=0; i<numSplines; ++i) {
    ...
    glm::vec3* squarePositions = new glm::vec3[uNumPoints];
    ...
    compute_catmull_rom_point(pointPositions, squarePositions, squareTrianglePositions, splines[i].points, currNumCtrlPts, i, prev_1_point, prev_2_point, next_1_point, connect_prev, connect_next);
    ...
    for (int j=0; j<uNumPoints; ++j) {
      // bottom right: right
      squareColors[j*4] = glm::vec4(splineBinormals[i][j].x, splineBinormals[i][j].y, splineBinormals[i][j].z, 1.0);
      // top right: top
      squareColors[j*4+1] = glm::vec4(splineNormals[i][j].x, splineNormals[i][j].y, splineNormals[i][j].z, 1.0);
      // top left: left
      squareColors[j*4+2] = glm::vec4(-splineBinormals[i][j].x, -splineBinormals[i][j].y, -splineBinormals[i][j].z, 1.0);
      // bottom left: bottom
      squareColors[j*4+3] = glm::vec4(-splineNormals[i][j].x, -splineNormals[i][j].y, -splineNormals[i][j].z, 1.0);
    }
    ...
    // compute_square_rail_idx(squareIndex, uNumPoints);
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3) * squareIdxCnt, squarePositions);
    // Upload color data
