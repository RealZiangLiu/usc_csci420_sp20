...
void compute_catmull_rom_point (glm::vec3* pointPositions, glm::vec3* squarePositions, Point* points, int currNumCtrlPts, int splineIdx, Point& prev_1, Point& prev_2, Point& next_1, int& pointCnt, bool connect_prev = false, bool connect_next = false) {
  pointCnt = 0;
...
void initScene(int argc, char *argv[])
{
  ...
  */
  ...
  for (int i=0; i<numSplines; ++i) {
    ...
    if (connect_prev) {
      uNumPoints += ((int)(1.0 / param_u_step) * 2);
    }
    if (connect_next) {
      uNumPoints += (int)(1.0 / param_u_step);
    }
    ...
    compute_catmull_rom_point(pointPositions, squarePositions, splines[i].points, currNumCtrlPts, i, prev_1_point, prev_2_point, next_1_point, realPointCnt, connect_prev, connect_next);
