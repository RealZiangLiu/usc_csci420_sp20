...
void loadCrossbarTexture (glm::vec3* crossbarTrianglePositions, int splineIdx) {
  ...
  int crossbar_vertex_cnt = crossbar_cnt[splineIdx] * 18; // 36 points per bar, divided by 2

  glm::vec2* crossbarTexCoords = new glm::vec2[crossbar_vertex_cnt];

  // Populate positions array
  for (int i=0; i<crossbar_cnt[splineIdx]; i+=2) {
    for (int j=0; j<6; ++j) { // loop over 6 faces
...
void add_cross_bar_points (vector<Point>& crossbarPositions, int splineIdx, int pointCnt) {
  ...
  // decide where to place crossbar
  for (int i=crossbar_dist / 2 + crossbar_width; i<pointCnt; i+=crossbar_dist) {
    ...
    // offsets to place crossbar under rails
...
void compute_t_rail_idx (glm::vec3* tTrianglePositions, glm::vec3* tPositions, int pointCnt, int splineIdx, bool left) {
  int currCnt = 0;
  // distance fractor of left and right rail from middle
  double mult = left? (-0.03) : 0.03;
  for (int i=0; i<pointCnt-1; ++i) {
    // left and right rail offsets
...
void initScene(int argc, char *argv[])
{
  ...
  */
  ...
  for (int i=0; i<numSplines; ++i) {
    ...
    // Compute total number of points along the spline
    int uNumPoints = ((int)(1.0 / param_u_step)) * (currNumCtrlPts - 3) + 1;

    GLuint currLeftVBO, currRightVBO, currLeftVAO, currRightVAO;
    GLuint currEBO;

    // second spline needs to be connected with the previous spline if in multi track mode
    ...
    // Add points for connection between splines
    if (connect_prev) {
      uNumPoints += ((int)(1.0 / param_u_step));
    }

    // Add points used for closing loop
    if (close_loop && i == numSplines - 1) {
      uNumPoints += ((int)(1.0 / param_u_step) * 3);
    }

    int tIdxCnt = 60 * (uNumPoints - 1);  // 10 surfaces, 6 vertices per surface
    ...
    glm::vec3* tPositions = new glm::vec3[uNumPoints * 10]; // 10 vertices per point on spline
    ...
    // factors to offset normal of slopes
